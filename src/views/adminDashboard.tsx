...
export default function AdminDashboard(props: Props) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setUserList(await pizzaService.listUsers(userPage, 5, '*'));
    })();
  }, [props.user, franchisePage, userPage]);

  async function filterUsers() {
    setUserList(await pizzaService.listUsers(userPage, 5, `*${filterUserRef.current?.value}*`));
  }
  ...
  if (Role.isRole(props.user, Role.Admin)) {
    response = (
      <View title="Mama Ricci's kitchen">
        <div className="text-start py-8 px-4 sm:pc-6 lg:px-8">
